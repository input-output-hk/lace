...
import { BlockfrostAssetProvider } from '../../../src/blockfrost';
...
describe('BlockfrostAssetProvider', () => {
  ...
  describe('getAsset', () => {
    const mockedAssetId = Cardano.AssetId(
    ...
    test('combines onchain and offchain metadata', async () => {
      mockResponses(request, [
        [`assets/${mockedAssetResponse.asset}`, { data: mockedAssetResponse }],
      ]);
      ...
      expect(response).toMatchObject<Asset.AssetInfo>({
        ...
        nftMetadata: {
          description: 'The Nut Coin',
          image: Asset.Uri(
            'ipfs://QmfKyJ4tuvHowwKQCbCHj4L5T3fSj8cjs7Aau8V7BWv226',
          ),
          name: 'My NFT token',
          version: '1.0',
        },
    ...
    test('description and image as array', async () => {
      mockResponses(request, [
        [
          'assets/8153f8be9f05b2f32b481bbf7af877f592160b39e87f5f55c8ab035f4e46543031',
          {
            data: {
              asset:
                '8153f8be9f05b2f32b481bbf7af877f592160b39e87f5f55c8ab035f4e46543031',
      ...
      expect(response).toMatchObject<Asset.AssetInfo>({
        ...
        fingerprint: Cardano.AssetFingerprint(
          'asset1h2z87pq2tr4ksxl5nkzd2ltrrtd6vvmf5nnn46',
        ),
    ...
    test('file src as array', async () => {
          ...
          `assets/${mockedAssetId}`,
          {
            data: {
              ...
              onchain_metadata: {
                ...
                files: [
                  {
                    image: 'should be in other properties',
                    mediaType: 'image/png',
                    src: ['http://', 'some.png'],
                  },
                ],
      ...
      expect(
        response.nftMetadata!.files![0].otherProperties?.get('image'),
      ).toBe('should be in other properties');
    });
