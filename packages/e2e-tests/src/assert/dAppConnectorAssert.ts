...
import AllDonePage from '../elements/dappConnector/allDonePage';
...
class DAppConnectorAssert {
  ...
  async assertSeeAllDonePage() {
    await this.assertSeeHeader();
    await AllDonePage.image.waitForDisplayed();

    await AllDonePage.heading.waitForDisplayed();
    await expect(await AllDonePage.heading.getText()).to.equal(
      await t('browserView.transaction.success.youCanSafelyCloseThisPanel')
    );

    await AllDonePage.description.waitForDisplayed();
    await expect(await AllDonePage.description.getText()).to.equal(await t('core.dappTransaction.signedSuccessfully'));

    await AllDonePage.closeButton.waitForDisplayed();
    await expect(await AllDonePage.closeButton.getText()).to.equal(await t('general.button.close'));
