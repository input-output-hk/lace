...
const KEYSTONE_HW_PREFIX = 'keystone-';
const XFP_PATTERN = /^[0-9a-f]{8}$/;

/**
 * Wallet ID for an air-gapped Keystone, derived from the device master
 ...
 */
export type HardwareKeystoneWalletId = Tagged<
  WalletId,
  'HardwareKeystoneWalletId'
>;

export const HardwareKeystoneWalletId = (
  xfpHex: string,
): HardwareKeystoneWalletId => {
  const normalized = xfpHex.toLowerCase();
  if (!XFP_PATTERN.test(normalized)) {
    throw new Error(
      `Keystone wallet id requires an 8 hex character master fingerprint, got "${xfpHex}"`,
    );
  }
  return `${KEYSTONE_HW_PREFIX}${normalized}` as HardwareKeystoneWalletId;
};
