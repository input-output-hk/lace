...
import {
  ...
  filterFrankenUtxos,
} from '../src/filter-franken-utxos';
import { CardanoPaymentAddress, CardanoRewardAccount } from '../src/types';
...
// A base address whose PAYMENT credential is a script, sharing the stake
// credential of LEGITIMATE_ADDRESS_1. This is the resting-DEX-order shape, and
// it is the only rejection branch the pre-submit signing guard cannot see:
// required-signer derivation skips script credentials.
const SCRIPT_ADDRESS = CardanoPaymentAddress(
  'addr_test1zqwk0nt6a2hdae87w0k240nuezf2fra52qgemksdm4m0jfts6cdlwymh7v9kr8cd8cy5vx8l7h6v9da84ml2cjd90fusvh0uye',
);
...
describe('extractPaymentCredential', () => {
  ...
  it('reports a script payment credential as ScriptHash', () => {
    const credential = extractPaymentCredential(SCRIPT_ADDRESS);
...
describe('filterFrankenUtxos', () => {
  ...
  it('rejects a script payment credential even under an owned stake credential', () => {
    const ownedCredentials = extractOwnedPaymentCredentials([
      createMockAddress(LEGITIMATE_ADDRESS_1),
    ]);

    const result = filterFrankenUtxos(
      [createMockUtxo(SCRIPT_ADDRESS)],
      ownedCredentials,
    );
  ...
  it('omits a script address from the owned credential set', () => {
    // A script address in the discovered set must not authorise anything: its
    // hash could otherwise collide into the owned set and re-admit script UTxOs.
    const ownedCredentials = extractOwnedPaymentCredentials([
      createMockAddress(SCRIPT_ADDRESS),
    ]);

    expect(ownedCredentials.size).toBe(0);
  });
