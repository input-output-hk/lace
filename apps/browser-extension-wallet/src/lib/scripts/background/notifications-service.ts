...
export class NotificationsService {
  ...
  subscribe(topic: Pick<NotificationsTopic, 'id'>): void {
    const subscription = this.pubnub.channel(topic.id).subscription();
    subscription.subscribe();
    this.topics = this.topics.map((t) => (t.id === topic.id ? { ...t, subscribed: true, subscription } : t));
    this.emitTopics();
  }

  unsubscribe(topic: Pick<NotificationsTopic, 'id'>): void {
    const topicToUnsubscribe = this.topics.find((t) => t.id === topic.id);
    if (topicToUnsubscribe) {
      topicToUnsubscribe.subscription?.unsubscribe();
      topicToUnsubscribe.subscription = undefined;
      topicToUnsubscribe.subscribed = false;
    }
    this.emitTopics();
  }
