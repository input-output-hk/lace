...
| L-205 Provider API keys bundled in the client        | No provider API key is bundled on mobile: Maestro routes through the IOG-hosted proxy (no key) and Blockfrost is moved behind it (the R2 L-204 proxy work, extended to mobile). The only remaining `EXPO_PUBLIC_*` values are the PostHog ingest token and Sentry DSN — public-by-design telemetry identifiers, not provider credentials.                                                                 |
