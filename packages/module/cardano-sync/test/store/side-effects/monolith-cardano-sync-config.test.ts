...
import { monolithCardanoSyncConfig } from '../../../src/store/side-effects/monolith-cardano-sync-config';

import type { MonolithCardanoSyncRoundContext } from '../../../src/store/side-effects/monolith-cardano-sync-config';
import type { CardanoWalletAccount } from '@lace-contract/cardano-context';
...
// SUT: the engine instantiated with the monolith round config. These cases are
// the behaviour-equivalence proof — identical inputs/assertions to the original
// coordinate-sync behaviour tests.
const coordinateCardanoSync = createCoordinateCardanoSync(
  monolithCardanoSyncConfig,
);

describe('monolithCardanoSyncConfig (coordinate engine)', () => {
  ...
  describe('buildRoundOperations', () => {
    const { buildRoundOperations } = monolithCardanoSyncConfig;
    const account0 =
      threeAccountCardanoWalletAccounts[0] as CardanoWalletAccount;

    const baseContext = (
      overrides: Partial<MonolithCardanoSyncRoundContext>,
    ): MonolithCardanoSyncRoundContext => ({
      accounts: [account0],
      chainId: Cardano.ChainIds.Preprod,
      accountsWithAddresses: new Set<AccountId>(),
      tipHash: 'tip-hash-bro',
      ...overrides,
    });

    it('returns a single ADDRESS_DISCOVERY op for an account with no addresses', () => {
      const ops = buildRoundOperations(
        account0,
        baseContext({ accountsWithAddresses: new Set<AccountId>() }),
      );

      expect(ops).toEqual([
        {
          operationId: `${account0.accountId}-tip-hash-bro-address-discovery`,
          status: 'Pending',
          description: 'sync.operation.address-discovery',
          startedAt: expect.any(Number) as unknown as Timestamp,
        },
    ...
    it('returns TRANSACTION_POLLING ops for an account that already has addresses', () => {
      const ops = buildRoundOperations(
        account0,
        baseContext({
          accountsWithAddresses: new Set<AccountId>([account0.accountId]),
        }),
      );

      expect(ops).toEqual([
        {
          operationId: `${account0.accountId}-tip-hash-bro-transaction-polling`,
          status: 'Pending',
          description: 'sync.operation.transaction-polling',
          startedAt: expect.any(Number) as unknown as Timestamp,
        },
    ...
    it('threads the ctx tipHash into the operation IDs (no-tip placeholder when undefined)', () => {
      const ops = buildRoundOperations(
        account0,
        baseContext({
          accountsWithAddresses: new Set<AccountId>(),
      ...
      expect(ops[0].operationId).toBe(
        `${account0.accountId}-no-tip-address-discovery`,
      );
    });
  });
});
