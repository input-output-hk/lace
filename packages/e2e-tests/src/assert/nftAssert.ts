import { NftList } from '../elements/NFTs/nftList';
...
class NftAssert {
  ...
  async assertCounterNumberMatchesWalletNFTs() {
    const nftsList = new NftList();
    const nftsNumber = await nftsList.getItemsCount();
  ...
  async assertSeeNftList(minimumNumberOfNfts: number) {
    ...
    await webTester.seeWebElement(nftList.container());

    await browser.waitUntil(async () => (await nftList.getItemsCount()) >= minimumNumberOfNfts, {
      timeout: 60_000,
      interval: 3000,
  ...
  async assertNftFallbackImage(nftName: string, contains: boolean) {
    const nftDetails = new NftDetails();
    const noImageSrc =
    ...
    await webTester.waitUntilSeeElement(nftDetails.imageOfSpecificNft(nftName), 15_000);
    const srcValue = await webTester.getAttributeValue(nftDetails.imageOfSpecificNft(nftName).toJSLocator(), 'src');
    await (contains ? expect(srcValue).to.equal(noImageSrc) : expect(srcValue).to.not.equal(noImageSrc));
  }

  async assertSeeNftItem(index: string) {
    const nftItem = new NftItem(index);
    await webTester.seeWebElement(nftItem.container());
    await webTester.seeWebElement(nftItem.image());
    await webTester.seeWebElement(nftItem.name());
  }

  async assertSeeEachNftItem() {
    const itemsCount = await new NftList().getItemsCount();
  ...
  async assertSeeNftDetails() {
    const nftDetails = new NftDetails();
    await webTester.waitUntilSeeElement(nftDetails.imageElement());
    await webTester.waitUntilSeeElement(nftDetails.infoSectionList().container());
    await webTester.waitUntilSeeElement(nftDetails.attributesSectionList().container());
  }

  async assertNftDisplayed(nftName: string, shouldBeDisplayed: boolean) {
    const nftItem = await $(new NftItem(nftName).toJSLocator());
    await nftItem.waitForDisplayed({ timeout: 20_000, reverse: !shouldBeDisplayed });
  }

  async assertSeeSendNFTButton(shouldBeDisplayed: boolean) {
    await new NftDetails().sendNFTButton.waitForDisplayed({ reverse: !shouldBeDisplayed });
