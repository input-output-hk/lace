...
export class TransactionBuilder {
  ...
  public build(): UnsignedTransaction {
    ...
    const feeRateSatoshis = (this.feeRate * 100_000_000) / 1_000;
    ...
    for (const utxo of selectedUTxOs) {
      ...
      this.psbt.addInput({
        ...
        witnessUtxo: {
          script: payments
            .p2wpkh({ pubkey: Buffer.from(knownAddr.publicKeyHex, 'hex'), network: this.net })
            .output!,
    ...
    const vBytes =
      ...
      coinselectOutputs.length * OUTPUT_SIZE +
