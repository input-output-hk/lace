...
import type { KeyPair, UnsignedTransaction } from '../../src/common';
...
describe('BitcoinSigner', () => {
  ...
  it('throws if private key is missing', () => {
    expect(
      () => new BitcoinSigner({ publicKey: Buffer.from(publicKey) } as KeyPair),
    ).toThrow('Private key is required to sign transactions.');
  });
...
describe('signTx', () => {
  it('signs and finalizes a PSBT, returning valid hex and signature', () => {
    ...
    const isValid = rePsbt.validateSignaturesOfInput(
      0,
      (pubkey, messageHash, sig) => ecc.verify(messageHash, pubkey, sig),
    );
    expect(isValid).toBe(true);
  });
});
