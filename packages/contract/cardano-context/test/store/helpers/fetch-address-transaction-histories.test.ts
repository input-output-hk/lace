...
describe('fetchAddressTransactionHistories', () => {
  ...
  it('emits Ok([]) without querying the provider when every address has already loaded its oldest entry', () => {
    // Without this emission the caller never receives a result, so the
    // account-level `hasLoadedOldestEntry` flag is never flipped and the
    // Activities-tab loader spins forever once `desired > loaded`.
