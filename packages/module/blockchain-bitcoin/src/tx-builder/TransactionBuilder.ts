...
export class TransactionBuilder {
  ...
  private readonly knownAddresses: DerivedAddress[];
   ...
   */
  public setUtxoSet(utxos: BitcoinUTxO[]): this {
    this.utxos = utxos;
    return this;
  }
   ...
   * @returns The builder instance for chaining.
   */
  public addOutput(address: string, value: number): this {
    if (
  ...
  public build(): UnsignedTransaction {
    ...
    const result = this.inputSelector.selectInputs({
      ...
      feeRate: feeRateSatoshis,
      hasOpReturn: !!this.opReturnData,
    });
    ...
    for (const utxo of selectedUTxOs) {
      const knownAddr = this.knownAddresses.find(
      ...
      this.psbt.addInput({
        ...
        witnessUtxo: {
          script: payments.p2wpkh({
            pubkey: Buffer.from(knownAddr.publicKeyHex, 'hex'),
            network: net,
          }).output!,
          value: utxo.satoshis,
        },
    ...
    coinselectOutputs.forEach(({ address, value }) => {
      if (isP2trAddress(address)) {
        const taprootPubKey = Buffer.from(
