...
export const useAddWalletHardwareSetup = ({
  ...
  const isTrezorOption =
    loadedHwBlockchainSupport
      ?.flat()
      .some(
        support =>
          support.deviceOptionId === optionId &&
          support.walletType === WalletType.HardwareTrezor,
      ) ?? false;

  // Safe 7 and newer only connect through the Trezor Suite desktop app, so
  // when Suite is unreachable the sheet warns upfront instead of letting the
  // user run into Trezor's dead-end popup. `null` (probe not applicable,
  // e.g. outside the extension) shows nothing.
