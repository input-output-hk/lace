...
const exposeNotificationsCenterAPI = async () => {
  ...
  const notificationsClient = new NotificationsClient({
    provider: {
      name: 'PubNub',
      configuration: {
        // TODO: Use Lace test subscribe key
        skipAuthentication: true,
        subscribeKey: 'sub-c-90e15f01-9708-4a33-b6b4-7033b0f6f4e9'
      }
    },
    storage: notificationsStorage,
    // TODO: Get user ID from posthog
    userId: 'c7c2e2c0-4e3a-4f22-9e55-6e4d3b0f03c8',
    ...
    onTopics: (newTopics) => {
      // eslint-disable-next-line no-console
      console.log('onTopics', newTopics);
