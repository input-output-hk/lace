...
import type { CborReader } from '../../ur-transport';
...
export const decodeHdKeyMap = (reader: CborReader): DecodedHdKey => {
  ...
  const [purposeComponent, coinTypeComponent, accountComponent] =
    origin.components;
  if (
    purposeComponent === undefined ||
    coinTypeComponent === undefined ||
    accountComponent === undefined
  ) {
    throw new Error('crypto-hdkey origin must have at least 3 components');
  }
  const purpose = purposeComponent.index;
  const coinType = coinTypeComponent.index;
  const account = accountComponent.index;
