...
import type { UrResult } from '../../ur-transport';
import type { CryptoHDKey, CryptoKeypath } from '@lace-lib/vendor';
...
const assertAccountPath = (path: DerivationPath): number => {
...
const parseAccountKey = (
  ...
  const hardenedAccount = assertAccountPath(path);
  ...
  return {
    path,
    accountIndex: hardenedAccount - HARDENED_OFFSET,
