...
describe('TransactionBuilder', () => {
  ...
  it('throws if no UTXOs are set', () => {
    ...
    builder.addOutput('tb1qujrdfmuk7xe7rmx8zzk5n6gyxhz8p84ynwv9l2', 1000);
  ...
  it('builds a balanced transaction successfully', () => {
    ...
    builder
      ...
      .addOutput(recipientAddress, 1000);
    ...
    const totalOutput = result.context.txOutputs.reduce(
      (accumulator, out) => accumulator + out.value,
      0,
    );
    ...
    const changeOutput = result.context.txOutputs.find(
      out => out.address === changeAddress,
    );
    expect(changeOutput).toBeDefined();
    expect(changeOutput?.value).toEqual(Number(totalInput - 1000 - fee));
  });
