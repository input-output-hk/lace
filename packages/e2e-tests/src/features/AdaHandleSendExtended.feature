...
Feature: ADA handle - extended view
  ...
  Scenario: Extended view - Ada handle transfer e2e, review flow
    ...
    And I'm sending the NFT with name: "$handletosend3" in extended mode
    ...
    And the Sent transaction is displayed with handle name: "$handletosend3" in extended mode
    When I navigate to Address Book extended page
    Then I see warning for address row with name "$handletosend3"
    And I hover over the warning icon for "$handletosend3" handle
    Then I see handle warning tooltip
    And I click "Send" button on page header
    When I enter "$handletosend3" in the bundle 1 recipient's address
    Then I see review handle banner for handle: "$handletosend3"
    When I click "Review" button in review handle banner
    Then I see review handle drawer in extended mode for handle: "$handletosend3"
    ...
    And I see address row with name "$handletosend3" and address "$handletosend3" on the list in extended mode
    And I validate that handle: "$handletosend3" is not listed on the Receive screen
