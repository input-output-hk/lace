...
export class PostHogClient {
    ...
    this.userIdService
      ...
      .then((id) => {
        posthog.init(this.getApiToken(this.chain), {
          ...
          disable_cookie: true,
          persistence: 'memory',
          // bootstrap the ID given by userIdService,
          // This will automatically send the distinct_id as property for every event, so, we don't need to call this.userIdService.getUserId(this.chain.networkMagic) every time we send an event
          // Also, we need this to load the correct flag, if we don't set this, a random ID generated by post hog is used, so, we are getting the flags for the post hog random ID instead of the flags for user ID generated by our userIdService
          bootstrap: {
            distinctID: id,
            isIdentifiedID: true
          },
  ...
  async updatePostHogConfig(chain: Wallet.Cardano.ChainId): Promise<void> {
    // we need this to update the distinct ID configured at initialization, we have to do this when the user opts out or opts in, since the ID changes depending on that
    const id = await this.userIdService.getUserId(chain.networkMagic);
    posthog.register({
      distinct_id: id
    });
    // we need to reload the flags since the flag condition could change depending on type of user tracking.
    // this will depend on how we are going to configure the experiment, so we might not need this
