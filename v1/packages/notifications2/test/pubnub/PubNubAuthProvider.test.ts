...
describe('PubNubAuthProvider', () => {
  ...
  describe('getToken', () => {
    it('should return AuthToken from successful response', async () => {
      ...
      // Expected result (stored in seconds)
      const expectedAuthToken: AuthToken = {
        token: 'test-token-12345',
        expiresAt: mockApiResponse.expiresAt, // Kept in seconds
        refreshMargin: 60
      };
    ...
    it('should use default error discriminator when none provided', async () => {
      ...
      const expectedAuthToken: AuthToken = {
        token: 'default-discriminator-token',
        expiresAt: mockApiResponse.expiresAt,
        refreshMargin: 60
      };
    ...
    it('should use default refreshMargin when not provided in response', async () => {
      ...
      expect(result.refreshMargin).toBe(60); // 60 seconds default
    });
    ...
    it('should cache token in storage after successful fetch', async () => {
      ...
      // Verify token was saved to storage (with expiresAt in seconds)
  ...
  describe('Token Caching', () => {
    ...
    it('should load token from storage if in-memory cache is empty', async () => {
      // Stored token from storage (stored in seconds)
    ...
    it('should fetch new token when cached token is expiring soon', async () => {
      // Expired token from storage (stored in seconds)
      const expiredToken: AuthToken = {
        token: 'expired-token',
        expiresAt: Math.floor(Date.now() / 1000) + 30, // Expires in 30 seconds
        refreshMargin: 60 // But refresh margin is 60 seconds
      };
    ...
    it('should bypass cache when forceRefresh is true', async () => {
      ...
      // Expected results (stored in seconds)
      const expectedToken1: AuthToken = {
        token: 'cached-token',
        expiresAt: mockApiResponse1.expiresAt,
        refreshMargin: 60
      };

      const expectedToken2: AuthToken = {
        token: 'new-token',
        expiresAt: mockApiResponse2.expiresAt,
        refreshMargin: 60
      };
  ...
  describe('Token Validation', () => {
    it('should consider token valid when expiry is beyond refresh margin', async () => {
      const validToken: AuthToken = {
        token: 'valid-token',
        expiresAt: Math.floor(Date.now() / 1000) + 300, // 5 minutes from now (seconds)
        refreshMargin: 60 // 1 minute margin (seconds)
      };
    ...
    it('should consider expired token invalid', async () => {
      // Stored expired token (stored in seconds)
      const expiredToken: AuthToken = {
        token: 'expired-token',
        expiresAt: Math.floor(Date.now() / 1000) - 1, // Already expired (1 second ago)
        refreshMargin: 60
      };
