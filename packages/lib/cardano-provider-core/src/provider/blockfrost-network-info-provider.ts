...
import { Cardano, Milliseconds, Seconds } from '@cardano-sdk/core';
...
export class BlockfrostNetworkInfoProvider
  ...
  implements NetworkInfoProvider
{
  ...
  public async ledgerTip(): Promise<Cardano.Tip> {
    try {
      const block = await this.request<Responses['block_content']>(
        'blocks/latest',
      );
      return BlockfrostToCardanoSDK.blockToTip(block);
    } catch (error) {
      throw toProviderError(error);
    }
  }
  ...
  public async protocolParameters(): Promise<Cardano.ProtocolParameters> {
    try {
      const response = await this.request<Responses['epoch_param_content']>(
        'epochs/latest/parameters',
      );
      return BlockfrostToCardanoSDK.protocolParameters(response);
    } catch (error) {
      throw toProviderError(error);
    }
  }
  ...
  protected async parseEraSummaries(
    summaries: Schemas['network-eras'],
    systemStart: Date,
  ): Promise<EraSummary[]> {
    try {
