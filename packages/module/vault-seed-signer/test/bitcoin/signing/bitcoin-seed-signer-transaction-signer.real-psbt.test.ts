...
/**
 * Stands in for the air-gapped device: parses the crypto-psbt request the signer
 * sends, signs every input with the supplied key, and returns the signed PSBT as
 * a crypto-psbt response. Records the request PSBT so the test can inspect the
 * key-origins the signer stamped.
 */
const installSigningDeviceStub = (
  ...
  airGappedQrExchangeHook.trigger = options =>
    new Observable<AirGappedQrExchangeResult>(subscriber => {
      const requestCbor = (options.request as { cbor: Uint8Array }).cbor;
      ...
      subscriber.next({
        ...
        cbor: encodeCryptoPsbt(psbt.toBuffer()),
      });
      subscriber.complete();
    });
...
describe('BitcoinSeedSignerTransactionSigner real-PSBT round trip', () => {
  ...
  it('stamps real bip32Derivation (xfp + native-segwit path) on the request PSBT', async () => {
    const signer = new BitcoinSeedSignerTransactionSigner({
      masterFingerprint: XFP_HEX,
    });

    await firstValueFrom(signer.sign({ serializedTx: dto.serializedTx }));
  ...
  it('finalizes and extracts a real signed transaction hex', async () => {
    ...
    const result = await firstValueFrom(
      signer.sign({ serializedTx: dto.serializedTx }),
    );

    const parsed = JSON.parse(HexBytes.toUTF8(result.serializedTx)) as {
      network: string;
      hex: string;
    };
