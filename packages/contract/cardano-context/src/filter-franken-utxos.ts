 ...
 *
 * Script addresses are excluded, so a UTxO at a ScriptHash payment credential is
 * rejected downstream even when it carries the user's stake key. Pairing a script
 * hash with a stake credential needs no secret, so presence under the user's
 * stake key is not evidence the user can spend it.
 *
...
export const extractOwnedPaymentCredentials = (
  addresses: readonly AnyBlockchainAddress<CardanoAddressData>[],
