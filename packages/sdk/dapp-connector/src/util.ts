import type { AuthenticatorApi } from './AuthenticatorApi';
import type { MethodRequestOptions } from '@lace-sdk/extension-messaging/src/types';
 ...
 */
export const cloneSender = ({
  frameId,
  id,
  tab,
  url,
}: Runtime.MessageSender): Runtime.MessageSender => ({
  frameId,
  id,
  tab: tab
    ? {
...
export const checkAccessRequestOptions = (
  ...
  validate: async (_, sender) => {
    const haveAccess =
      sender && (await authenticator.haveAccess(cloneSender(sender)));
