...
type BlockfrostTxUtxoContent = Responses['tx_content_utxo'];
type BlockfrostInputs = Responses['tx_content_utxo']['inputs'];
type BlockfrostInput = Pick<
  Unpacked<BlockfrostInputs>,
  'address' | 'amount' | 'output_index' | 'tx_hash'
>;
type BlockfrostOutputs = Responses['tx_content_utxo']['outputs'];
type BlockfrostOutput = Unpacked<BlockfrostOutputs>;
export type BlockfrostTransactionContent = Unpacked<
  Responses['address_transactions_content']
>;
export type BlockfrostTxDetails = Responses['tx_content'];
export type BlockfrostUtxo = Unpacked<BlockfrostAddressUtxoContent>;

export const BlockfrostToCardanoSDK = {
  addressUtxoContent: (
    address: string,
    ...
    [
      BlockfrostToCardanoSDK.hydratedTxIn(
        BlockfrostToCardanoSDK.inputFromUtxo(address, utxo),
      ),
      BlockfrostToCardanoSDK.txOut(
        BlockfrostToCardanoSDK.outputFromUtxo(address, utxo),
        txOutFromCbor,
      ),
    ] as Cardano.Utxo,

  /**
   * Transform Blockfrost account-based UTxOs (accounts/{stake}/utxos)
   * into Cardano SDK UTxO tuples.
   * Note: This endpoint does not include inputs or collateral information.
   */
  accountUtxos: (utxos: Responses['account_utxo_content']): Cardano.Utxo[] =>
    utxos.map(utxo =>
      BlockfrostToCardanoSDK.addressUtxoContent(utxo.address, utxo),
    ),
  ...
  inputs: (inputs: BlockfrostInputs): Cardano.TxIn[] =>
    inputs.map(input => BlockfrostToCardanoSDK.hydratedTxIn(input)),
  ...
  outputs: (outputs: BlockfrostOutputs): Cardano.TxOut[] =>
    outputs.map(output => BlockfrostToCardanoSDK.txOut(output)),
  ...
  protocolParameters: (
    ...
    costModels: new Map<Cardano.PlutusLanguageVersion, Cardano.CostModel>([
      [
        ...
        Object.values(
          blockfrost.cost_models!.PlutusV1 as { [key: string]: number },
        ),
      ],
      [
        Cardano.PlutusLanguageVersion.V2,
        Object.values(
          blockfrost.cost_models!.PlutusV2 as { [key: string]: number },
        ),
      ],
    ]),
    ...
    governanceActionDeposit: blockfrost.gov_action_deposit
      ? Number(blockfrost.gov_action_deposit)
      : undefined,
    ...
    protocolVersion: {
      major: blockfrost.protocol_major_ver,
      minor: blockfrost.protocol_minor_ver,
    },
  ...
  txOut: (
    blockfrost: BlockfrostOutput,
    txOutFromCbor?: Cardano.TxOut,
  ): Cardano.TxOut => {
    const value: Cardano.Value = {
      coins: BigInt(
        blockfrost.amount.find(({ unit }) => unit === 'lovelace')!.quantity,
      ),
    };
    ...
    if (blockfrost.inline_datum)
      txOut.datum = Serialization.PlutusData.fromCbor(
        HexBlob(blockfrost.inline_datum),
      ).toCore();
    if (blockfrost.data_hash)
      txOut.datumHash = Hash32ByteBase16(blockfrost.data_hash);

    if (txOutFromCbor?.scriptReference)
      txOut.scriptReference = txOutFromCbor.scriptReference;

    return txOut;
  },
  transactionUtxos: (
    utxoResponse: BlockfrostTxUtxoContent,
    txContent?: Cardano.Tx,
  ) => {
    const collaterals = utxoResponse.inputs
      .filter(input => input.collateral)
      .map(BlockfrostToCardanoSDK.hydratedTxIn);
    ...
    const outputPromises: Cardano.TxOut[] = utxoResponse.outputs
      ...
      .map(output => {
        const foundScript = txContent?.body.outputs.find(
          o => o.address === output.address,
        );

        return BlockfrostToCardanoSDK.txOut(output, foundScript);
      });
