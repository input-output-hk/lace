...
const discardStaleTabs = async (currentTabId: number) => {
  const allTabs = [
    ...(await brandPotentialExtApiError(tabs.query({ title: 'Lace' }), 'Failed to query for stale lace tabs')),
  ...
  allTabs.forEach((tab) => {
    ...
    void brandPotentialExtApiError(tabs.discard(tab.id), 'Failed to discard stale tab');
  });
};
