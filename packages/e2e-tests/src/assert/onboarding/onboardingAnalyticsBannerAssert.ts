...
class OnboardingAnalyticsBannerAssert {
  ...
  async assertBannerIsDisplayedCorrectly() {
    ...
    expect(await AnalyticsBanner.agreeButton.getText()).to.equal(
      await t('package.core.confirmationBanner.agree', 'core')
    );
    await AnalyticsBanner.rejectButton.waitForDisplayed();
    expect(await AnalyticsBanner.rejectButton.getText()).to.equal(
      await t('package.core.confirmationBanner.reject', 'core')
    );
