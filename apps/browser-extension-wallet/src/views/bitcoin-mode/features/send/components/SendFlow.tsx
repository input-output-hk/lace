/* eslint-disable @typescript-eslint/no-shadow, no-magic-numbers, max-params, unicorn/no-null, promise/catch-or-return, promise/no-nesting, max-statements, @typescript-eslint/no-non-null-assertion */
...
export const SendFlow: React.FC<SendFlowProps> = ({ updateSubtitle }) => {
  ...
  // Step 2 -> 3
  const goToReview = () => {
    const unsignedTransaction = buildTransaction(
      knownAddresses,
      knownAddresses[0].address,
      address,
      feeRate,
      btcStringToSatoshisBigint(amount),
      utxos,
      network!
    );
  ...
  const handlePasswordSubmit = (password: string) => {
    signTransaction(unsignedTransaction!, walletInfo!.encryptedSecrets.seed, Buffer.from(password))
      .then((signedTx) => {
        bitcoinWallet
          .submitTransaction(signedTx.hex)
          .then((hash) => {
  ...
  if (step === 'PASSWORD') {
    updateSubtitle('');
    return <PasswordInput onSubmit={handlePasswordSubmit} onBack={backToReview} />;
  }

  if (step === 'DONE') {
    updateSubtitle('');
    return <TransactionSuccess hash={confirmationHash} />;
  }
