  ...
  return {
    ...
    message$: local$.pipe(
      map(
        (data): PortMessage => ({
          data,
          port: { postMessage: message => postMessage(message).subscribe() },
        }),
      ),
...
const addOne = async (argument: bigint) => argument + 1n;
interface SetUpProps {
  destructor?: Destructor;
  nestedSomeNumbers$?: Observable<bigint>;
  someNumbers$?: Observable<bigint>;
}
const baseChannel = ChannelName('base-channel');
const setUp = ({
  ...
  const api = {
    ...
    nonExposedMethod: vi.fn(async () => true),
    nonExposedObservable$: throwError(() => new Error('Shouldnt be called')),
    someNumbers$,
  };

  type FullApi = typeof api;
  type ExposedApi = Omit<
    FullApi,
    'nestedNonExposed' | 'nonExposedMethod' | 'nonExposedObservable$'
  >;
  const api$ = new Subject<FullApi | null>();
  ...
  const properties: RemoteApiProperties<ExposedApi> = {
    ...
    addOneTransformedToAddTwo: {
      ...
      requestOptions: {
        transform: request => ({
          ...request,
          args: [(request.args[0] as bigint) + 1n],
        }),
      },
    },
    factory: {
  ...
  vi.spyOn(api.nonExposedObservable$, 'subscribe');
  return {
    api,
    api$,
    cleanup: () => {
...
describe('remoteApi integration', () => {
  ...
  it('invalid properties are undefined', () => {
    sut = setUp();
    expect(
      (sut.consumer as unknown as Record<string, unknown>).addTwo,
    ).toBeUndefined();
  });
  ...
  it('shuts down underlying messenger when the exposed object is shut down', async () => {
    ...
    await expect(
      firstValueFrom(sut.hostMessenger.message$),
    ).rejects.toThrowError(EmptyError);
  });
  ...
  describe('methods returning promise', () => {
    ...
    it('accessing property returns the same function object', () => {
      ...
      expect(sut.consumer.addOneTransformedToAddTwo).toBe(
        sut.consumer.addOneTransformedToAddTwo,
      );
    });

    describe('nested property', () => {
      it('accessing property returns the same function object', () => {
        expect(sut.consumer.nested.addOneNoZero).toBe(
          sut.consumer.nested.addOneNoZero,
        );
        expect(sut.consumer.nested.nestedSomeNumbers$).toBe(
          sut.consumer.nested.nestedSomeNumbers$,
        );
      });
      ...
      it('requestOptions | validate', async () => {
        await expect(async () =>
          sut.consumer.nested.addOneNoZero(0n),
    ...
    describe('top-level property', () => {
      ...
      it('requestOptions | transform', async () => {
      ...
      it('calls remote method from new source and resolves result', async () => {
        ...
        otherApi.addOne = async (argument: bigint) => argument + 11n;
    ...
    it('throws RemoteApiShutdownError when the remote object shutdown method is called more than once', async () => {
      ...
      await expect(sut.consumer.addOne(1n)).rejects.toThrowError(
        RemoteApiShutdownError,
      );
    });
  });
  ...
  describe('observables', () => {
    ...
    beforeEach(() => {
      ...
      sut = setUp({
        nestedSomeNumbers$: nestedSomeNumbersSource$,
        someNumbers$: someNumbersSource$,
      });
      sut.emitInitial();
    });
    ...
    describe('top level property', () => {
      describe('mirrors source emissions and completion', () => {
        it('values emitted after subscription', async () =>
          new Promise<void>(done => {
            const emitted = firstValueFrom(
              sut.consumer.someNumbers$.pipe(toArray()),
            );
            setTimeout(async () => {
        ...
        it('replays 1 last value emitted before subscription', async () =>
          new Promise<void>(done => {
            someNumbersSource$.next(-1n);
            someNumbersSource$.next(0n);
            setTimeout(() => {
              const emittedFromSubscriptionBeforeConnect = firstValueFrom(
                sut.consumer.someNumbers$.pipe(toArray()),
              );
              sut.hostMessenger.connect();
              setTimeout(() => {
                const emittedFromSubscriptionAfterConnect = firstValueFrom(
                  sut.consumer.someNumbers$.pipe(toArray()),
                );
                setTimeout(async () => {
                  someNumbersSource$.next(1n);
                  ...
                  expect(await emittedFromSubscriptionBeforeConnect).toEqual([
                    0n,
                    1n,
                  ]);
                  expect(await emittedFromSubscriptionBeforeConnect).toEqual(
                    await emittedFromSubscriptionAfterConnect,
                  );
                  done();
                }, 1);
              }, 1);
      ...
      describe('mirrors source errors', () => {
        const subscribeAndAssertError = (
          done: (value: PromiseLike<void> | void) => void,
        ) => {
        ...
        it('when thrown after subscription', async () =>
          new Promise<void>(done => {
            subscribeAndAssertError(done);
            someNumbersSource$.error(new Error('err'));
          }));

        it('when thrown before subscription', async () =>
          new Promise<void>(done => {
            someNumbersSource$.error(new Error('err'));
            subscribeAndAssertError(done);
          }));
    ...
    it('nested property | mirrors source emissions and completion for values emitted after subscription', async () =>
      new Promise<void>(done => {
        const emitted = firstValueFrom(
          sut.consumer.nested.nestedSomeNumbers$.pipe(toArray()),
        );
        setTimeout(async () => {
    ...
    describe('changing source', () => {
      describe('top level property', () => {
        it('mirrors values emitted from original source, then from new source', async () =>
          new Promise<void>(done => {
            const emitted = firstValueFrom(
              sut.consumer.someNumbers$.pipe(toArray()),
            );
        ...
        it('replays 1 last value emitted by new source before subscription', async () =>
          new Promise<void>(done => {
            someNumbersSource$.next(-1n);
            sut.api$.next(otherApi);
        ...
        it('mirrors new source errors', async () =>
          new Promise<void>(done => {
            sut.consumer.someNumbers$.subscribe({
              error: error => {
                expect(error instanceof Error).toBe(true);
        ...
        it('nested property | mirrors new source emissions and completion for values emitted', async () =>
          new Promise<void>(done => {
            const emitted = firstValueFrom(
              sut.consumer.nested.nestedSomeNumbers$.pipe(toArray()),
            );
        ...
        it('ignores old source emissions', async () =>
          new Promise<void>(done => {
            const emitted = firstValueFrom(
              sut.consumer.someNumbers$.pipe(toArray()),
            );
        ...
        it('does not emit if source was disabled', async () =>
          new Promise<void>(done => {
            someNumbersSource$.next(1n);
            const emittedBeforeDisable = firstValueFrom(
              sut.consumer.someNumbers$,
            );
            setTimeout(async () => {
              ...
              sut.api$.next(null);
              setTimeout(async () => {
                const emittedAfterDisable = firstValueFrom(
                  race(sut.consumer.someNumbers$, of('nothing').pipe(delay(1))),
                );
        ...
        it('rejects new api object with missing properties', async () => {
          otherApi.someNumbers$ =
            null as unknown as (typeof otherApi)['someNumbers$'];
          sut.api$.next(otherApi);
          await expect(
            firstValueFrom(sut.consumer.someNumbers$),
          ).rejects.toThrowError();
        });
      });
    });
  ...
  describe('factory', () => {
    ...
    const testFactory = (
      ...
      describe(factoryName, () => {
        ...
        it('garbage collected consumed api shuts down the messengers', async () =>
          new Promise<void>(done => {
            const areFactoryMessengersShutdown = () => {
              // This is likely to break on lodash update
              // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-explicit-any
              const cache = createSubjects.cache as any;
              // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
              const stringCache = cache.__data__;
              // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
              const dataCache = stringCache.string;
              // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
              const entriesData = dataCache.__data__ as Record<string, unknown>;
              const [_, subjects] = Object.entries(entriesData).find(([key]) =>
                key.includes('-factory'),
              )!;
              const { consumerMessage$, hostMessage$ } = subjects as ReturnType<
                typeof createSubjects
              >;
        ...
        it.todo(
          'garbage collected consumed api enables remote object to be garbage collected',
        );
      });
    };
