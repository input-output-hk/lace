...
class NftAssert {
  ...
  async assertCounterNumberMatchesWalletNFTs() {
    const nftsNumber = await NftItem.containers.length;
  ...
  async assertNftFallbackImage(nftName: string, contains: boolean) {
    ...
    const nftImage = await NftItem.getNftImageByName(nftName);
  ...
  async assertSeeNftItem(index: number) {
    const nft = await NftItem.containers[index];
    await nft.waitForDisplayed();
    await nft.$(await NftItem.image.selector).waitForDisplayed();
    await nft.$(await NftItem.name.selector).waitForDisplayed();
  }

  async assertSeeEachNftItem() {
    const itemsCount = await NftItem.containers.length;
