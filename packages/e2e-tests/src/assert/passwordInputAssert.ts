...
class PasswordInputAssert {
  async assertPasswordFieldValueIsHidden() {
    ...
    await expect(await PasswordInput.input.getAttribute('type')).to.equal('password');
    await expect(await PasswordInput.input).to.not.haveOwnProperty('value');
  }

  async assertPasswordFieldValueIsDisplayed(expectedValue: string) {
    await PasswordInput.input.waitForDisplayed();
    await expect(await PasswordInput.input.getValue()).to.equal(expectedValue);
  }
  ...
  async assertSeePasswordInputError(expectedErrorMsg: string) {
    ...
    await expect(error).to.equal(expectedErrorMsg);
  }
