...
When(
  /^I (navigate to|am on) (Tokens|NFTs|Transactions|Staking|Address Book|Settings) (extended|popup) page$/,
  async (
    _ignored: string,
    targetPage: 'Tokens' | 'NFTs' | 'Activity' | 'Staking' | 'Settings' | 'Address Book',
    mode: 'extended' | 'popup'
  ) => {
    await visit(targetPage, mode);
  }
);
