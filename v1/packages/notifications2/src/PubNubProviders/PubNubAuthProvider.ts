...
export class PubNubAuthProvider implements NotificationsAuthProvider {
   ...
   * Expects response format: { token: string, expiresAt: number (seconds), refreshMargin?: number (ms) }
   * Converts expiresAt from seconds to milliseconds for consistency with Date.now().
  ...
  private fetchTokenFromEndpoint(): Observable<AuthToken> {
    return from(
      (async () => {
        ...
        // Use refreshMargin from response if provided, otherwise default to 1 minute
        const refreshMargin =
          typeof data.refreshMargin === 'number'
            ? data.refreshMargin
            : PubNubAuthProvider.DEFAULT_REFRESH_MARGIN_SECONDS * PubNubAuthProvider.MS_PER_SECOND;

        return {
          token: data.token,
          // Convert expiresAt from seconds to milliseconds
          expiresAt: data.expiresAt * PubNubAuthProvider.MS_PER_SECOND,
          refreshMargin
        };
      })()
    );
