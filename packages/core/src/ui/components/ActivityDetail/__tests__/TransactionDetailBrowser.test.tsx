...
import { ActivityDetailBrowser, ActivityDetailBrowserProps } from '../ActivityDetailBrowser';
import '@testing-library/jest-dom';

const transactionDate = '2021/09/10';

describe('Testing ActivityDetailsBrowser component', () => {
  const addrListProps: ActivityDetailBrowserProps = {
    isPopupView: false,
  ...
  test('should display transaction hash and copy button', async () => {
    const { findByTestId } = render(<ActivityDetailBrowser {...addrListProps} />);
  ...
  test('should display transaction inputs and outputs list', async () => {
    const { findByTestId } = render(<ActivityDetailBrowser {...addrListProps} addrOutputs={[]} />);
  ...
  test('should display transaction metadata if available', async () => {
    const { findByTestId } = render(
      <ActivityDetailBrowser {...addrListProps} metadata={[{ key: '1', value: [{ msg: 'Metadata' }] }]} />
    );
  ...
  test('should not display transaction metadata if not available', async () => {
    const { queryByTestId: query } = render(<ActivityDetailBrowser {...addrListProps} />);
