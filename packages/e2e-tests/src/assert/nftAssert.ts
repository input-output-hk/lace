...
class NftAssert {
  ...
  async assertNftFallbackImage(nftName: string, contains: boolean) {
    ...
    await webTester.waitUntilSeeElement(new NftItem(nftName).image(), 15_000);
    const srcValue = await webTester.getAttributeValue(new NftItem(nftName).image().toJSLocator(), 'src');
