...
import styles from './ProgrammableAssetTable.module.scss';
import { useTranslation } from 'react-i18next';
import { ImageWithFallback } from '../ImageWithFallback';
import { IRow } from '../AssetTable';
...
export type ProgrammableAssetTableProps = {
  rows: IRow[];
  totalItems: number;
...
export const ProgrammableAssetTable = ({
  rows,
  onLoad,
  scrollableTargetId,
  totalItems,
  onRowClick,
  popupView
}: ProgrammableAssetTableProps): React.ReactElement | null => {
  if (!rows || rows.length === 0 || popupView) {
    return null;
  }
  ...
  const columns: ColumnsType<IAssetColumn> = useMemo(() => {
    const result = [
      {
        title: popupView ? '' : 'Programmable Token',
        dataIndex: 'token',
        key: 'token',
        width: '45%'
      },
